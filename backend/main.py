...
async def delete_comment(comment_id: int, db: Session = Depends(get_db)):
    ...
    """
    # First, get the comment to ensure it exists
    comment = db.query(Comment).filter(Comment.id == comment_id).first()
    if not comment:
        raise HTTPException(status_code=404, detail="Comment not found")
    
    # Get the comment ID to use for finding replies
    target_comment_id = comment.id
    
    # Find all replies to this comment (comments with parent_comment_id = target_comment_id)
    replies = db.query(Comment).filter(Comment.parent_comment_id == target_comment_id).all()
    
    # Delete all replies and their likes
    for reply in replies:
        # Delete likes for each reply first
        db.query(CommentLike).filter(CommentLike.comment_id == reply.id).delete()
        # Delete the reply
        db.delete(reply)
    
    # Delete all likes for the main comment
    db.query(CommentLike).filter(CommentLike.comment_id == target_comment_id).delete()
